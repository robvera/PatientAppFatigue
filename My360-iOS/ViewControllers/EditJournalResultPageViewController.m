...
- (void)check4Message:(PatientJournal*) patientJournal{
    switch (appID ) {
        case Diabetes:
            break;
        case MM:
...
- (void) saveQuestionnaire:(PatientJournalQuestionnaire * ) patientJournalQuestionnaire patientJournal:(PatientJournal *)  patientJournal{
    
    patientJournalQuestionnaire.patientJournalId = patientJournal.id;
    patientJournalQuestionnaire.applicationQuestionnaireId = [[ConfigurationManager sharedManager] appID];
    
    [patientJournalQuestionnaire createAsync:^(id object, NSError *error) {
...
-(void) populatePatientJournal4Individual:(PatientJournal *)patientJournal{
    ...
    switch ([[[ConfigurationManager sharedManager] appID] integerValue] ) {
        ...
        case NSCLC:
            ...
            //patientJournal.notes = self.intensityTextField.text;
            ...
            //        case Asthma:
            //            //  ** I M P O R T A N T --- No break sends control to next case statement , ie ASTHMA
            //            NSLog(@"this   Asthma");
        case AAPA: Asthma:
            NSLog(@"this is AAPA & Asthma");
...
// *********************************************
// Written to Combine all SAVES  - svaz 4/29/15
// *********************************************
- (BOOL) saveJournal:(id) sender {
    __block BOOL isSuccessfulSave = NO;
    [self pushBusyOperation];
    
    
    
    NSLog(@" editmode = %d & somethingChanged = %d",self.editMode ,self.somethingChanged);
    ...
    if (self.editMode && self.somethingChanged) {
        ...
        //    GENERIC CODE FOR ALL APPS    - saveMerge - svaz 4/30/15
        
        self.patientJournal.ability = self.ability;
        
        //Wasn't saving this 3 so i added  RV 5/6/15
        ...
        //Add to save changes on NotetectView --rvera 5/12/15
        if (![self.noteTextView.text isEqualToString:kFieldPlaceHolderText]) {
        ...
        //   Method to populate Journal only for individual apps     - SaveMERGE svaz 4/30/15
        [ self populatePatientJournal4Individual:self.patientJournal];
        ...
        [self.patientJournal updateAsync:^(id object, NSError *error) {
           ...
           [self popBusyOperation];
            ...
            if (error) {
                ...
                [self check4Message:self.patientJournal];
                
                self.somethingChanged = NO; // svaz added for success
                
                //--Comment this cause the View was coming back to the EditJournal instead of going to the next view Treatment Logs --rvera 5/14/15
        ...
        //***  Commented in favor of UpdateAsyn above     - SaveMERGE svaz 4/30/15
    ...
    //*************************************
        ...
        if (self.journalOnTextField.text.length == 0) {
        ...
        else if (self.moodIdTextField.text.length == 0 &&  appID != RA){
           ...
           // }
            ...
            [self populatePatientJournal4Individual:patientJournal];
            ...
            [patientJournal createAsync:^(id obj, NSError *error) {
                ...
                if (error) {
                    ...
                    self.patientJournal = patientJournal; // for update, in case of  save from nav bar
                    if (self.patientQuestionnaire) {
                        //  user went into the questionnaire view
                        // *** EVERY NEW Jopurnal MUST have a patientQuestionnaire
                        [self saveQuestionnaire:self.patientQuestionnaire patientJournal:patientJournal];
                        self.somethingChanged = NO; // svaz added for success
                    } else {
                        ...
                        [self saveQuestionnaire: patientQuestionnaire patientJournal:patientJournal];
                        self.somethingChanged = NO; // svaz added for success
                    }
                    
                    [self check4Message:patientJournal];
...
- (void)logsButtonTapped:(id)sender
{
    ...
    //      [self performSegueWithIdentifier:@"TreatmentLogs" sender:self];
    //   }else{
    //       [self performSegueWithIdentifier:@"TreatmentLogsNew" sender:self];
    //  }
}
...
- (BOOL) savedTapped4Logs{
    ...
    //  Rewired for other apps      - SaveMERGE svaz 4/30/15
    return [self saveJournal:@"DoNotClose"];
}
...
- (void)saveTapped {
    ...
    if (!self.somethingChanged) {
        [self.navigationController popViewControllerAnimated:YES];
        [self dismissViewControllerAnimated:YES completion:nil];
        return; // exit nothing changed on VC so return
    }else{
    //  Rewired for other apps      - SaveMERGE svaz 4/30/15
    [self saveJournal:nil];
    }
//    if ([self saveJournal:nil] ) {
//        [self.navigationController popViewControllerAnimated:YES];
//        [self popBusyOperation];
//        
//        return ;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSLog(@" editMode = %d", self.editMode);
