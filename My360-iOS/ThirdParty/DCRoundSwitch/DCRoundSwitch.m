...
//            -  MSAA cleanup svaz 11/27/14
    NSDictionary *fontAttributes = @{NSFontAttributeName: self.toggleLayer.labelFont};
	CGFloat width = [onString sizeWithAttributes:fontAttributes].width;
	CGFloat offWidth = [offString  sizeWithAttributes:fontAttributes].width;
...
- (void)setOn:(BOOL)newOn animated:(BOOL)animated ignoreControlEvents:(BOOL)ignoreControlEvents
{
    ...
    //   -  MSAA cleanup svaz 11/27/14
    // ADDED so that the YES/NO layers do not bleed!!!
    if (!self.on) {
        self.toggleLayer.onString = @"";
    }else{
        self.toggleLayer.onString = @"YES";
    }
    //        end    -  MSAA cleanup svaz 11/27/14
...
- (void)setOnTintColor:(UIColor *)anOnTintColor
{
	if (anOnTintColor != onTintColor)
	{
        ...
        //            -  MSAA cleanup svaz 11/27/14 - this is a transparent Layer!!!
        self.toggleLayer.onTintColor =  [UIColor colorWithRed:66.0/255.0f green:211.0/255.0f blue:80.0/255.0f alpha:1.0];
//        self.toggleLayer.onTintColor = [UIColor greenColor];

        //     END - change color of switch       -  MSAA cleanup svaz 11/27/14
		[self.toggleLayer setNeedsDisplay];
	}
}
