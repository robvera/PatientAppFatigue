...
@implementation AllergensTodayData
- (id)init {
    self = [super init];
    if (self) {
        ...
        _dataKeys = [NSArray arrayWithObjects:@"AllergensTodayData", nil];
        
        //init categories
        _categories = [[NSArray alloc] init];
        
        _chartType = CHART_TYPE_PIE;
...
-(void)generateData {
    if ([AppDelegate hasConnectivity])
    {
        ...
        NSArray *objects = [Chart query:@"allergens_today_report" params:queryAttributes error:&error];
