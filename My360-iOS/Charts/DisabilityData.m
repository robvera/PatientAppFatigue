...
-(void)generateData {
    if ([AppDelegate hasConnectivity])
    {
        ...
        NSArray *objects = [Chart query:@"disability_report" params:queryAttributes error:&error];
        ...
        if (error == nil && objects.count > 0){
            ...
            //Read out all of the ability data in the right order
            for(Ability *ab in ad.abilityData){
                [cats addObject:ab.name];
            }
            _categories = [[cats reverseObjectEnumerator] allObjects];
        }
        else {
